const Scanner = {
  ...
  _isNonLeakable(value, patternId) {
    ...
    if (patternId === 'env-pattern') {
      ...
      if (/^(token|refresh_token|access_token)\s*=\s*["']?[a-f0-9]{8,40}["']?$/i.test(value)) return true;
      if (/^[A-Z_]+\s*=\s*["']?[a-f0-9]{6,40}["']?$/i.test(value)) return true;
      if (/=\s*["']?[a-f0-9]{32}["']?$/i.test(value)) return true;
      if (/(?:^|&)(?:token|refresh_token|access_token|id_token)=[a-zA-Z0-9_%-]{6,}/i.test(value)) return true;
      if (/&[a-z_]+=\w+/i.test(value) && /^[a-z_]+\s*=\s*\w+(?:&\w+=\w+)+/i.test(value)) return true;
      const kw = value.match(/^[A-Z_]+/i);
      if (kw) {
        let after = value.slice(kw[0].length).replace(/^\s*=*\s*/, '');
        if (after) {
          if (/^[a-z]\w*[.\s)[\];,=!?:><{}()]/.test(after)) return true;
          if (/^[,!?{}()[\];]/.test(after)) return true;
          if (/^(true|false|null|undefined|NaN)\b/.test(after)) return true;
          if (/^function\b/.test(after)) return true;
          if (/^\d+\s*[,}\]]/.test(after)) return true;
          if (/\.{3,}/.test(after)) return true;
          if (/^your[-_]/.test(after)) return true;
          if (/^(lmstudio|ollama|local)\b/.test(after)) return true;
          if (/^==/.test(after)) return true;
          if (/^[a-zA-Z_]\w*$/.test(after) && after.length < 50) return true;
          if (/(%[0-9A-F]{2}){3,}/i.test(after)) return true;
        }
      }
    }
    if (patternId === 'exposed-token') {
      if (/[Mm]odule__[a-zA-Z]+__/.test(value)) return true;
      if (/\.concat\(/.test(value)) return true;
      const etMatch = value.match(/["']([^"']+)["']/);
      if (etMatch) {
        const inner = etMatch[1];
        if (/^[a-z]+(?:[A-Z][a-z]+)+$/.test(inner)) return true;
        if (/^[a-z]+(?:t|d|s|ed|ing|ly)$/.test(inner) || inner.length < 5) return true;
        if (/^[A-Z][a-z]+[A-Z]/.test(inner)) return true;
        if (/^X-[a-zA-Z-]+$/.test(inner)) return true;
        if (/^data-[a-z-]+$/.test(inner)) return true;
        if (/^[a-z_]+$/.test(inner) && inner.length < 30) return true;
        if (/[a-z][A-Z]/.test(inner) && !/[0-9_-]{3,}/.test(inner)) return true;
      }
    }
    if (patternId === 'openai') {
      const keyBody = value.replace(/^sk-/, '');
      if (/^[a-zA-Z]+$/.test(keyBody)) return true;
      if ((keyBody.match(/[0-9]/g) || []).length < 3) return true;
    }
    if (patternId === 'generic-api-key' && /AIza[0-9A-Za-z_-]{35}/.test(value)) return true;
    if (patternId === 'hardcoded-password') {
      const pwMatch = value.match(/["']([^"']+)["']/);
      if (pwMatch) {
        const inner = pwMatch[1];
        if (inner.startsWith('/')) return true;
        if (/^[a-z]+(?:[A-Z][a-z]+)+$/.test(inner)) return true;
        if (/^\w+(Create|Reset|Forget|Login|Signup)/.test(inner)) return true;
        if (/(?:password|pwd|token)\b/.test(inner) && inner.length < 40) return true;
      }
    }
    const q = value.match(/["']([^"']+)["']/);
    if (q) {
      const inner = q[1].toLowerCase();
      if ((patternId === 'exposed-token' || patternId === 'generic-api-key') &&
          /^(authorization|bearer|www-authenticate|x-api-key|x-auth-token|csrf-token)$/.test(inner)) return true;
      if ((patternId === 'exposed-token' || patternId === 'generic-api-key') &&
          /^usr_[a-z_]+$/.test(inner)) return true;
      if ((patternId === 'exposed-token' || patternId === 'generic-api-key') &&
          /^data-[a-z-]+$/.test(inner)) return true;
      if (patternId === 'exposed-token' &&
          /:\d{10,}/.test(inner)) return true;
    }
    if (/^(api[_-]?key|api[_-]?secret|token|secret|password|passwd|pwd|credential|auth_key)\s*[:=]\s*["'](password|secret|token|key|api_key|test|testpage|example|changeme|placeholder|csrf-token|bearer)["']$/i.test(value)) return true;
    try {
    ...
    if (patternId === 'env-pattern') {
      ...
      if (/^token\s*=\s*["']?[a-zA-Z0-9_-]{10,30}["']?$/i.test(value) && !/[{}]/.test(value)) return true;
      if (/^[a-z]+\s*=\s*(getCookie|getItem|session_token|req\.)/i.test(value)) return true;
      if (value.includes(';') && /;\s*(Domain|Path|Expires|HttpOnly|Secure|Max-Age|SameSite)\s*=/i.test(value)) return true;
      if (/^[a-z_]+\s*=\s*["']?[a-f0-9-]{36}["']?$/i.test(value)) return true;
    }
    if (patternId === 'exposed-token') {
      if (/token\s*=\s*["']AF5tSO[a-zA-Z0-9_-]+:\d+["']/.test(value)) return true;
      if (/TOKEN:\s*["']data-[a-z-]+["']/.test(value)) return true;
      if (/^\s*token\s*=\s*["'][a-zA-Z0-9_-]{10,40}:\d+["']/.test(value)) return true;
    }
    if (patternId === 'generic-api-key' && /apiKey:\s*["'][a-f0-9]{32}["']/.test(value)) {
      return true;
    }
    ...
    if (patternId === 'jwt') {
      ...
      if (parts.length === 3) {
        try {
          ...
          if (decoded.aud === 't' || decoded.aud === 'cdn' || decoded.aud === 'image') return true;
          if (typeof decoded.time === 'number') return true;
          if (decoded.sub && decoded.sub.length > 30 && !decoded.uid && !decoded.user_id && !decoded.email) return true;
          if (decoded.type === 'download' || decoded.type === 'upload' || decoded.type === 'access') return true;
        } catch {}
      }
    }
    if (patternId === 'stripe-live' || patternId === 'stripe-test') {
      try {
        const b64 = value.replace(/^p[kks]_live_/, '').replace(/^p[kks]_test_/, '');
    ...
    for (const pattern of PATTERNS) {
      try {
        ...
        while ((match = pattern.regex.exec(text)) !== null) {
          ...
          if ((pattern.id === 'replicate' || pattern.id === 'huggingface') &&
              match.index > 0 && /[a-zA-Z0-9_-]/.test(text[match.index - 1])) continue;
          findings.push({
            patternId: pattern.id,
            name: pattern.name,
            category: pattern.category,
            severity: pattern.id === 'google-ai' ? 'info' : pattern.severity,
            value: value,
