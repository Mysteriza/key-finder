const Scanner = {
  ...
  _isNonLeakable(value, patternId) {
    ...
    if (patternId === 'env-pattern') {
      ...
      if (kw) {
        let after = value.slice(kw[0].length).replace(/^\s*/, '');
        after = after.replace(/^=+/, '');
        if (after) {
  ...
  _isOwnGoogleKey(value, url) {
    ...
    const aiMatch = value.match(/AIza[0-9A-Za-z_-]{35}/);
    if (!aiMatch) return false;
    return true;
  },
    ...
    for (const pattern of PATTERNS) {
      try {
        ...
        while ((match = pattern.regex.exec(text)) !== null) {
          ...
          if (this._isOwnGoogleKey(value, url)) continue;
